...
!         enddo
       ...
       integer i, j  
       integer icolor, istart

!      === u_{i,j} = 1/4 (h^2 f_{i,j} + u_{i,j-1} +u_{i-1,j}+u_{i+1,j}+u_{i,j+1})

!      === icolor =0: red nodes, =1: black nodes
       do icolor=0, 1

!$omp parallel do private (i,istart)  
         do j=1, M
           if (mod(j + icolor, 2) .eq. 0) then
             istart = 2
           else 
             istart = 1 
           endif
...
!      === End of color loop
       ...
       dmax = 0.0
!$omp parallel do private(i,dtemp) reduction(max: dmax) 
       do j=1, M
         do i=1, M
